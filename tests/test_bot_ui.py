...
def test_about_text_contains_version_and_runtime_without_secrets() -> None:
    ...
    assert "1.5.2-alpha.1" in about
    assert "Linux/Ubuntu 24.04 or Docker" in about
    assert "TOKEN" not in about


def test_persian_menu_button_labels() -> None:
    labels = [
        button.text
        for row in menu_keyboard("fa").inline_keyboard
        for button in row
    ]
...
def test_persian_url_action_button_labels() -> None:
    labels = [
        button.text
        for row in url_action_keyboard("abc12345", "fa").inline_keyboard
        for button in row
    ]

    assert labels == [
        text("url_screenshot_button", "fa"),
        text("url_pdf_button", "fa"),
        text("url_html_button", "fa"),
